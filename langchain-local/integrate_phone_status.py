"""ScrapingPhoneStatusとBenchmarkのデータを統合してpklファイルに保存するスクリプト"""
import os
import pickle
import re
import tempfile
...
class DynamoDB:
    ...
    __slots__ = ["__dynamodb", "table", "__phone_column_name", "__cpu_column_name"]
    ...
    def fetch_all(self) -> list[dict[str, str]]:
        # DynamoDBから全データを取得
        benchmark_dicts = self.table.scan()["Items"]
    ...
    def fetch_by_name(self, phone_name: str) -> list[dict[str, str]]:
        ...
        benchmark_dicts = self.table.scan(FilterExpression=Attr(self.__phone_column_name).eq(phone_name))["Items"]
        if not benchmark_dicts:
            raise ValueError("DynamoDB ScrapingPhoneStatus: データがありません")
        return benchmark_dicts
    ...
    def fetch_by_contained_cpu(self, cpu: str) -> list[dict[str, str]]:
        # cpuが含まれるデータを取得
        return self.table.scan(FilterExpression=Attr(self.__cpu_column_name).contains(cpu))["Items"]
...
class PhoneIdentifier:
    ...
    def select_phone(self, benchmark_phone_names: list[str]) -> str:
        ...
        prompt = PromptTemplate.from_template(
            """以下のリストから、指定されたスマートフォンモデル({phone_name})に最も近いスマートフォンモデル名を選択してください。選択したモデル名はリスト内のものでなければなりません。リスト外のモデル名を選択しないでください。選択したモデル名のみを回答として提供してください。
...
def integrate_phone_dict(
    ...
    benchmark_dict_cp["CPUベンチマーク"] = benchmark_dict_cp.pop("Multicore")
    del benchmark_dict_cp["Name"]
    del benchmark_dict_cp["Processor"]
    del benchmark_dict_cp["SingleCore"]
    answer_dict = phone_dict | benchmark_dict_cp
    answers.append(answer_dict)


if __name__ == "__main__":
    scraping_phone_status_table = DynamoDB(TABLE_NAME)
    benchmark_table = DynamoDB(BENCHMARK_TABLE_NAME)
    # answers: 統合後のデータ, match_list: benchmarkがマッチしたかどうか
    answers: list[dict[str, str]] = []
    match_list: list[bool] = []

    # DynamoDBからデータを取得
    phones: list[dict[str, str]] = scraping_phone_status_table.fetch_all()
    ...
    for phone_dict in phones:
        ...
        print(f"No.{len(match_list)+1}: {cleaned_phone_name}")
        ...
        if cleaned_phone_name in benchmark_phone_names:
            ...
            benchmark_dicts = benchmark_table.fetch_by_name(cleaned_phone_name)
        ...
        # 同一の場合統合
        if is_phone_equal:
            benchmark_dicts = benchmark_table.fetch_by_name(benchmark_phone_name)
            integrate_phone_dict(benchmark_dicts[0], phone_dict, answers)
            # マッチ率の計算用
            match_list.append(True)
            print("\n---------------------------------------------------\n")
        ...
        benchmark_dicts: list[dict[str, str]] = benchmark_table.fetch_by_contained_cpu(phone_cpu)
        ...
        phone_dict_cp = phone_dict.copy()
        phone_dict_cp["GHz"] = ""
        phone_dict_cp["CPUベンチマーク"] = ""
        answers.append(phone_dict_cp)
    ...
    # 統合したデータをpickleで保存
    with open("integrated_phone_status.pickle", "wb") as f:
